...
export function computeBMR(input = {}) {
  ...
  // 自动选择（面向新手，无需手动决策）：
  //   填了有效身高 → Mifflin-St Jeor（含身高，更精确）；
  //   未填身高   → Henry 2005（仅体重，SPEC §4.1）。
  // 仍允许调用方显式传入 "mifflin" / "henry" 覆盖；选了 mifflin 但缺身高时回退 Henry。
  const heightPresent = Number.isFinite(height) && height > 0;
  let usedEquation;
  if (bmrEquation === "mifflin" || bmrEquation === "henry") {
    usedEquation = bmrEquation; // 显式覆盖优先
  } else {
    usedEquation = heightPresent ? "mifflin" : "henry"; // 默认按身高自动
  }
  if (usedEquation === "mifflin" && !heightPresent) usedEquation = "henry";
